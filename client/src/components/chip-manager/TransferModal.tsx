...
export const TransferModal: React.FC<TransferModalProps> = ({ isOpen, onClose, onTransfer, allPlayers, currentUserBalance, currentUserId }) => {
  ...
  const handleConfirm = async () => {
    if (parsedAmount <= 0 || !toUserId) return;
    setIsTransferring(true);
    try {
      await onTransfer(toUserId, parsedAmount);
      setAmount('');
      setToUserId('');
      onClose();
    } finally {
  ...
  const otherPlayers = allPlayers.filter(p => p.userId._id !== currentUserId);

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="TRANSFER CHIPS">
      <div className="space-y-6 font-mono">
        <div>
          <label className="block text-xs font-bold text-muted uppercase tracking-widest mb-2">FROM</label>
          <div className="text-lg font-bold text-white">YOU <span className="text-muted text-sm ml-2">({currentUserBalance.toLocaleString()} CHIPS)</span></div>
        </div>

        <div>
          <label className="block text-xs font-bold text-muted uppercase tracking-widest mb-2">TO</label>
          <select 
            className="w-full bg-surface-2 border border-border text-white rounded-xl px-4 py-4 outline-none focus:border-accent focus:shadow-glow font-sans transition-all appearance-none"
            value={toUserId}
        ...
        <div className="pt-4 border-t border-border flex justify-between items-end">
          <div>
            <div className="text-xs font-bold text-muted uppercase tracking-widest mb-1">NEW BALANCE</div>
        ...
        <div className="flex gap-4 pt-4">
          ...
          <button 
            ...
            disabled={isTransferring || parsedAmount <= 0 || !toUserId || newBalance < 0}
